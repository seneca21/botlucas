...
// Configuração do multer para upload direto para S3 (sem definir ACL)
...
const upload = multer({
    storage: multerS3({
        ...
        // Não especificar 'acl' para evitar erro de ACL
    })
});

// Importa o Sequelize e os modelos
...
// Sessão (lembrando que MemoryStore não é recomendado para produção)
app.use(session({
...
// (A função makeDay e getDetailedStats permanecem inalteradas)
// ... [Código de estatísticas permanece conforme seu atual] ...
...
app.post('/admin/bots', checkAuth, upload.single('videoFile'), async (req, res) => {
    try {
        ...
        // O arquivo foi enviado para o S3; o multer salvará a key no campo "filename"
        let videoFilename = '';
        if (req.file) {
            videoFilename = req.file.key; // OBS.: multer-s3-v3 armazena a key aqui
        }
...
// [POST] Editar bot existente (upload opcional de novo vídeo para S3)
