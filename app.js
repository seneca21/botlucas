...
//------------------------------------------------------
// getDetailedStats (igual antes, não removemos nada)
//------------------------------------------------------
async function getDetailedStats(startDate, endDate, originCondition, botFilters = []) {
    // (sem mudanças, aproveitamos seu código anterior)
    // ...
    // O conteúdo é o mesmo que você colou, sem alterações,
    // pois a diferença está no cálculo do startDate/endDate
    let totalUsers = 0;
    ...
    try {
        if (originCondition === 'main') {
            ...
            // ... (caso sem originCondition => estatísticas gerais)
            // ...
            const baseWhere = { pixGeneratedAt: { [Op.between]: [startDate, endDate] } };
            if (botFilters.length > 0 && !botFilters.includes('All')) {
                baseWhere.botName = { [Op.in]: botFilters };
...
//------------------------------------------------------
// /api/bots-stats => utiliza makeDayBrasilia(...) no lugar
//                    de makeDay(...)
//------------------------------------------------------
    ...
    try {
        ...
        if (dateRange) {
            ...
            switch (dateRange) {
                ...
                case 'custom': {
                    if (customStart) {
                        startDate = makeDayBrasilia(new Date(customStart));
                    } else {
                        startDate = makeDayBrasilia(new Date());
                    }
                    if (customEnd) {
        ...
        // Estatísticas últimos 7 dias (sempre no fuso de Brasília)
        const stats7Days = [];
        for (let i = 6; i >= 0; i--) {
