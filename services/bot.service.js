...
function initializeBot(botConfig) {
  ...
  bot.action(/^remarketing_select_plan_(\d+(\.\d+)?)$/, async (ctx) => {
    ...
    try {
      ...
      // *** ADIÇÃO IMPORTANTE ***:
      // Criamos um registro na tabela Purchase com status = "pending"
      // para representar o Pix gerado, mas ainda não pago.
      const newPurchase = await Purchase.create({
        userId: user ? user.id : null,
        planName: plan.name,
        planValue: plan.value,
        botName: botConfig.name,
        purchasedAt: null, // só definimos quando for pago
        status: 'pending', // gerado mas não pago
        originCondition: remarketingCond,
        pixGeneratedAt: new Date() // Data em que geramos esse PIX
      });

      // Armazenamos info na sessão
      ...
      // Guardamos o ID do purchase para atualizá-lo depois
  ...
  bot.action(/^select_plan_(\d+)$/, async (ctx) => {
    ...
    try {
      ...
      // *** ADIÇÃO IMPORTANTE ***:
      // Criamos um registro na tabela Purchase com status = "pending"
      // para representar o Pix gerado, mas não confirmado.
      const newPurchase = await Purchase.create({
        userId: user ? user.id : null,
        ...
        status: 'pending',
        purchasedAt: null // ainda não
      });

      userSessions[chatId].chargeId = chargeId;
      // Guardamos o ID do purchase para atualizar quando for pago
    ...
    try {
      ...
      if (paymentStatus.status === 'paid') {
        ...
        if (user) {
          ...
          // Aqui agora ao confirmar pagamento, em vez de "Purchase.create", vamos "UPDATE" o purchase que criamos "pending"
    ...
    try {
      ...
      if (paymentStatus.status === 'paid') {
        ...
        if (user) {
          ...
          // Update do purchase "pending" para "paid"
          if (session.purchaseId) {
            await Purchase.update(
              {
                status: 'paid',
