...
const BotModel = db.BotModel; // Modelo dos bots
...
// =====================================
// RATE LIMITING e proteções (mesmo código já ajustado)
// =====================================
...
function initializeBot(botConfig) {
  ...
  bot.action(/^select_plan_(\d+)$/, async (ctx) => {
    ...
    try {
      ...
      await ctx.reply('⚠️ Depois de pagar, clique em "Verificar Pagamento".',
  ...
  bot.action(/^remarketing_select_plan_(\d+(\.\d+)?)$/, async (ctx) => {
    // Tratamento similar para remarketing (mantido conforme versão anterior)
    // ...
    await ctx.answerCbQuery();
  });

  bot.command('status_pagamento', async (ctx) => {
    // Tratamento para verificação de pagamento (mantido conforme versão anterior)
  });
  ...
  // Rotinas de limpeza para rate limits e flood protection (mantidas)
